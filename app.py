...
# ==========================================
# Using absolute paths ensures stability on Render's environment
...
# ==========================================
app = Flask(__name__)
# Permanent secret key for session stability
    ...
    for i, s_path in enumerate(sessions):
        ...
        for attempt in range(2):
            ...
            try:
                ...
                emit_log(f"🔄 {basename}: proxy connection failed.")
        ...
        try:
            ...
            if action == 'health':
                ...
                reason_map = {'1': types.InputReportReasonSpam(), '2': types.InputReportReasonViolence(), '3': types.InputReportReasonPornography(), '4': types.InputReportReasonChildAbuse(), '5': types.InputReportReasonCopyright(), '8': types.InputReportReasonFake(), '9': types.InputReportReasonOther()}
            ...
            if str(e) != "STOPPED": emit_log(f"⚠️ {basename}: execution error.")
        finally:
            await client.disconnect()
    ...
    for idx, sfile in enumerate(sessions, start=1):
        ...
        try:
            ...
            async def otp_listener(event):
                ...
                otp_event.set(); await client.disconnect()
...
def index():
    ...
    proxies = open(PROXIES_FILE, 'r').read() if os.path.exists(PROXIES_FILE) else ""
    conf = load_config()
    return render_template('index.html', stats=stats, proxies=proxies, bot_running=IS_BOT_RUNNING, bot_token=conf.get('bot_token', ''))
...
def clear_logs():
    LOG_HISTORY.clear(); return jsonify({"status": "ok"})
...
def save_bot_token():
    conf = load_config(); conf['bot_token'] = request.json.get('token', ''); save_config(conf)
    return jsonify({"status": "ok"})
...
def start_bot():
    ...
    BOT_THREAD = threading.Thread(target=init_bot_engine, args=(token,)); BOT_THREAD.daemon = True; BOT_THREAD.start()
    return jsonify({"status": "ok"})

@app.route('/api/bot/set_webhook', methods=['POST'])
def set_webhook():
    token = load_config().get('bot_token', '')
    if not token: return jsonify({"status": "error"})
    webhook_url = request.url_root.replace('http://', 'https://') + 'webhook'
    r = requests.get(f"https://api.telegram.org/bot{token}/setWebhook?url={webhook_url}")
    if r.status_code == 200: emit_log(f"🌐 WEBHOOK CONNECTED: {webhook_url}")
...
def remove_webhook():
    ...
    emit_log("🗑️ WEBHOOK DELETED."); return jsonify(r.json())
