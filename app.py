...
def run_bot_thread(token):
    ...
    try:
        ...
        IS_BOT_RUNNING = True
        emit_log("🤖 OTP BOT STARTED.")
        BOT_APP.run_polling(stop_signals=())
    except Exception as e: emit_log(f"⚠️ OTP BOT ERROR: {e}")
    finally: IS_BOT_RUNNING = False
